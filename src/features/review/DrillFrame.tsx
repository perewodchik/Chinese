 ...
 */
export function useDrillRun(ids: ItemId[], skill: Skill) {
  ...
  return { at, id: ids[at] as ItemId | undefined, total: ids.length, log, answer };
}
    ...
    <div className="drill-done">
      ...
      {missed.length > 0 && (
        <>
          ...
          {skill !== 'sound' && (
            <CollectionPicker
              placeholder={`Put these ${missed.length} on paper…`}
              onPick={putOnPaper}
              style={{ maxWidth: 320 }}
            />
          )}
